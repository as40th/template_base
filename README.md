...
## MCP-SERVER (улучшение качества работы AI-ассистента)
...
Базовый сценарий agentic-coding'а:

- Пишем системный промпт для AI-ассистента.
- Проводим ручное ревью кода и при необходимости дорабатываем код вручную либо с помощью Ai-ассиетнта с помощью дополнительных промптов.
- Запускаем агента для генерации тестов `/test-generator`
- Проводим ручное ревью кода и при необходимости дорабатываем код вручную либо с помощью Ai-ассиетнта с помощью дополнительных промптов.
- Запускаем агента для проверки качества кода `/code-reviewer`, `/security-scanner`, `/pylancer`
- Запускаем агента для генерации документации `/documentation-generator`
...
* Линтинг запускаем вручную субагентом `pyright-scaner`
* Для этого должен быть установлен pyright
```bash
uv add --dev pyright
```
...
* Для интеграционных тестов требуется запуск внешних сервисов в контейнерах. Рекомендуется использовать Podman или Docker.
* На Windows потребуется WSL для запуска контейнеров.
